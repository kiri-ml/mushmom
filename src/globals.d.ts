...
interface InitialStatsHistoryResult<TPoint> { points: TPoint[]; latestPayload: StatsPayload; manifest: StatsManifest; }
interface ArchiveStatsHistoryResult<TPoint> { points: TPoint[]; chunks: StatsManifestChunk[]; }
interface LoadInitialStatsHistoryOptions<TPoint> { archiveBaseUrl?: string; latestUrl?: string; manifest?: StatsManifest; normalizePayload: (payload: StatsPayload) => TPoint[]; onInitial?: (payload: InitialStatsHistoryResult<TPoint>) => void; fetcher?: (url: string) => Promise<unknown>; }
interface LoadArchiveStatsHistoryOptions<TPoint> { archiveBaseUrl?: string; latestPayload: StatsPayload; manifest: StatsManifest; normalizePayload: (payload: StatsPayload) => TPoint[]; onArchive?: (payload: ArchiveStatsHistoryResult<TPoint>) => void; fetcher?: (url: string) => Promise<unknown>; }
